...
from enc_dec import enc, dec
import messages as _
import settings
from factory_registers import register
...
if platform == 'linux':
    # don't know how to set the requirement in buildoer.spec.. mostly for
    # getting rid of a warning anyway
    from linux_support import readQR
    import cv2
...
class mTag(App):
    ...
    def GPSstart(self, minTime, minDistance, widget = None):
        if (
            (self.gps_status == 'pre-init' and self.GPSinit())
            or self.gps_status not in (_.msg_nogps, 'pre-init',)
        ):
            try:
                ...
                self.gps_location = _.msg_nogps
                widget.text = _.msg_nogps
        else:
            widget.state = "normal"
            widget.text = _.msg_nogps

    def GPSstop(self):
        if self.gps_status not in (_.msg_nogps, 'pre-init',):
            try:
                gps.stop()
            except NotImplementedError:
                self.gps_location = _.msg_nogps
    ...
    def choose_file(self, button):
        ...
        if platform == 'android':
            ...
            content = F.ImportDialog(callback=self.import_file, button=button)
            _popup = Popup(title=_.msg_select_file, content=content,
                                size_hint=(0.9, 0.9))
    ...
    def save_picture(self, camera, filename):
        ...
        for w in self.root.get_screen("entries").walk():
            if type(w) is F.EntryRow and w.id == self.target_entry["id"]:
                for ww in w.walk():
